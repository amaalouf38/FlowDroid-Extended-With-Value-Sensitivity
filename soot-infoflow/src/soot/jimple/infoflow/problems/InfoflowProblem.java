...
import intervalanalysis.flowsets.Utility;
import stringanalysis.flowsets.SUtility;
...
import stringanalysis.flowsets.StringTaints.StringTaint;
import stringanalysis.flowsets.StringTaints.StringTaintsHandler;
    ...
    public FlowFunctions<Unit, Abstraction, SootMethod> createFlowFunctionsFactory() {
        return new FlowFunctions<Unit, Abstraction, SootMethod>() {
            ...
            private void addTaintViaStmt(final Abstraction d1, final AssignStmt assignStmt, Abstraction source,
                ...
                //beginning of section array with interval analysis
                try {
                    SootMethod m = interproceduralCFG().getMethodOf(assignStmt);

                    //if (Utility.isArray(leftValue) && Utility.isArray(rightValue)) {
                    //  if(newAbs.getAccessPath().getPlainValue() == leftValue &&source.getAccessPath().getPlainValue() == rightValue ) {

                    //      if (source.lstTaintedIntervalsForLocal.size() > 0) {
                    //         for (ArrayIndiceInterval t : source.lstTaintedIntervalsForLocal) {
                    //             newAbs.add(new ArrayIndiceInterval(t.lower, t.upper, t.bottom));
                    //          }
                    //         newAbs.reason = source.reason;
                    //      }
                    //      logger.info("==duplicate refs no taint ====" + assignStmt.toString() + " newSource:" + newAbs.toString()+ " source:" + source.toString());
                    //  }
                    // }

                    if (leftValue instanceof ArrayRef) {
                        if (!(rightValue instanceof NewArrayExpr)) {
                            //a[i] = RHS and RHS is not new array()
                            Value leftBase = ((ArrayRef) leftValue).getBase();
                            Value leftIndex = ((ArrayRef) leftValue).getIndex();
                            Abstraction arrayAbs = null;

                            if (newAbs != null)
                                arrayAbs = newAbs.getAccessPath().getPlainValue() == leftBase ? newAbs : null;

                            ArrayIndiceInterval e = null;
                            if (leftIndex instanceof Local) {

                                List<Unit> callStack = new ArrayList<>();
                                Abstraction crAbs = newAbs;
                                while (crAbs != null) {
                                ...
                                e = new ArrayIndiceInterval(OutputIntervalAnalysis.outputIntervalAnalysis(assignStmt, (Local) leftIndex,callStack));
                            } else if (leftIndex instanceof NumericConstant) {
                                Double rvalue = new Double(((NumericConstant) leftIndex).toString());
                                e = new ArrayIndiceInterval((int) Math.floor(rvalue), (int) Math.ceil(rvalue), false);
                            }
                            //right value is tainted
                            if (arrayAbs != null) {
                                arrayAbs.add(e);
                                logger.info("==add ====" + assignStmt.toString() + " e:" + e.toString() + " newSource:" + newAbs.toString());
                            }
                        }
                        ...
                        Abstraction arrayAbs = null;
                        if (newAbs != null)
                            arrayAbs = newAbs.getAccessPath().getPlainValue() == leftValue ? newAbs : null;

                        if (arrayAbs != null) {
                            arrayAbs.empty();
                            logger.info("==empty ====" + assignStmt.toString() + " newSource:" + newAbs.toString());
                        }
                    }
                } catch (Exception ex) {
                    logger.error("==Exception ====" + ex.toString());
                }
                //end of section array with interval analysis
            ...
            private Set<Abstraction> createNewTaintOnAssignment(final AssignStmt assignStmt, final Value[] rightVals,
                ...
                //uncomment for interval analysis
                ...
                if (!addLeftValue && !aliasOverwritten) {
                    for (Value rightVal : rightVals) {
                        ...
                        // generic case, is true for Locals, StringRefs that are
                        // equal etc..
                ...
                return new NotifyingNormalFlowFunction((Stmt) src) {
                    ...
                    public Set<Abstraction> computeTargetsInternal(Abstraction d1, Abstraction source) {
                        ...
                        Set<Abstraction> res = propagationRules.applyNormalFlowFunction(d1, newSource, stmt,
                                (Stmt) dest, killSource, killAll, interproceduralCFG().getMethodOf(stmt));
                        if (killAll.value)
                        ...
                        if (src instanceof AssignStmt) {
                            ...
                            /*//beginning of section array with interval analysis
                            Set<Abstraction> resInt = new HashSet<Abstraction>();
                            final Value leftValue = assignStmt.getLeftOp();
                            final Value rightValue = assignStmt.getRightOp();
                            boolean addLeftValue = resAssign != null;
                            ...
                            try {
                                ...
                                if (leftValue instanceof ArrayRef) {
                                    if (!(rightValue instanceof NewArrayExpr)) {
                                        ...
                                        if (addLeftValue) {
                                            for (Abstraction a : resAssign) {
                                                if (a.getAccessPath().getPlainValue() == leftBase) {
                                                    arrayAbs = a;
                                                    break;
                                                }
                                            }
                                        } else
                                            arrayAbs = newSource.getAccessPath().getPlainValue() == leftBase ? newSource : null;

                                        ArrayIndiceInterval e = null;
                                        if (leftIndex instanceof Local) {
                                            e = new ArrayIndiceInterval(OutputIntervalAnalysis.outputIntervalAnalysis(assignStmt, (Local) leftIndex));
                                        } else if (leftIndex instanceof NumericConstant) {
                                            Double rvalue = new Double(((NumericConstant) leftIndex).toString());
                                            e = new ArrayIndiceInterval((int) Math.floor(rvalue), (int) Math.ceil(rvalue),false);
                                        }
                                        if (addLeftValue) {
                                            //right value is tainted
                                            arrayAbs.add(e);
                                            logger.info("==add ====" + assignStmt.toString()+ " e:" + e.toString()  + " newSource:" + newSource.toString());
                                        }
                                    }
                                } else if (rightValue instanceof NewArrayExpr) {
                                    //a=new array
                                    ...
                                    if (addLeftValue) {
                                        for (Abstraction a : resAssign) {
                                            if (a.getAccessPath().getPlainValue() == leftValue) {
                                                arrayAbs = a;
                                                break;
                                            }
                                        }
                                    } else
                                        arrayAbs = newSource.getAccessPath().getPlainValue() == leftValue ? newSource : null;
                                    ;

                                    Local reasonOfVisit = newSource.getAccessPath().getPlainValue();
                                    Local resonOfTaint = arrayAbs != null ? arrayAbs.reason : null;

                                    if (resonOfTaint != null && resonOfTaint == reasonOfVisit) {
                                        arrayAbs.empty();
                                        logger.info("==empty ====" + assignStmt.toString()  + " newSource:" + newSource.toString());
                                    }
                                }
                            } catch (Exception ex) {
                                logger.error("==Exception ====" + ex.toString());
                            ...
                            //end of array indice sensitivity*/
