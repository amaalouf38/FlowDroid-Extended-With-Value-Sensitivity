 ...
 */public class ValueArraySparseSet extends AbstractFlowSet {
	...
	public Iterator<Object> iterator()
	{
		Object[] copiedElements = new Object[numElements];
		System.arraycopy(elements, 0, copiedElements, 0, numElements);
		return Arrays.asList(copiedElements).iterator();
	}
	...
	public void removeStringValFor(soot.Local local) {
		int i = 0;
		while (i < this.numElements) {
			if (((StringVal)elements[i]).l.getName()==local.getName()) {
				elements[i] = elements[--numElements];
				return;
			} else
				i++;
	...
	public void merge(ValueArraySparseSet in,boolean isDummy)
	{
		int i = 0;
		while (i < in.numElements) {
			fStringVal curFInterval=(fStringVal)in.elements[i];
			soot.Local l=new soot.jimple.internal.JimpleLocal(curFInterval.l.getName(), RefType.v("java.lang.String"));
			StringVal inter = new StringVal(l, (curFInterval.a!=null?curFInterval.a.clone():null), curFInterval.bottom, curFInterval.top,isDummy);
			this.add(inter);
	...
	public boolean hasBottom() {
		int i = 0;
		while (i < this.numElements) {
			if(((StringVal)elements[i]).isBottom()) {
				return true;
			}
			i++;
		}
	...
	public void setBottom() {
		int i = 0;
		while (i < this.numElements) {
			((StringVal)elements[i]).bottom=true;
			i++;
		}
