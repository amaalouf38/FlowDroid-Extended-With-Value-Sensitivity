...
public class Context<M,N,A> implements soot.Context, Comparable<Context<M,N,A>> {
	...
	private Table<N, N,Boolean> cevals= HashBasedTable.create();
	...
	public void setEdgeConvergenceValue(N node, N succ,Boolean T)
	{
		if (this.cevals == null) {
			this.cevals=HashBasedTable.create();
		}

		this.cevals.put(node, succ, T);
	}
	public Boolean getEdgeConvergenceValue(N node, N succ)
	{
		return this.cevals.get(node, succ);
	}
	/**
