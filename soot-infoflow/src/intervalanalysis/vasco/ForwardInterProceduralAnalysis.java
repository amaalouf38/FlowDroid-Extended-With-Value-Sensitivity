 ...
 */
package intervalanalysis.vasco;
...
public abstract class ForwardInterProceduralAnalysis<M, N, A> extends InterProceduralAnalysis<M, N, A> {
    ...
    public Set<CallSite<M, N, A>> getCallersOfAContext(Context<M, N, A> currentContext) {
        return contextTransitions.getCallers(currentContext);
    }
    public Map<M,Context<M, N, A>> getCallersOfAContext(CallSite<M, N, A> callSite) {
    ...
    public void doAnalysis() {
        ...
        while (!worklist.isEmpty()) {
            ...
            if (verbose) {
                System.out.println("WorkList = " + currentContext.getWorkList().toString());
                System.out.println("---------------------------------------");
            }
            ...
            if (node != null) {
                ...
                if (predecessors.size() != 0) {
                    ...
                    for (N pred : predecessors) {
                        ...
                        A predOut = currentContext.getEdgeValue(pred, node);

                        if (predOut == null) {
                            predOut = currentContext.getValueAfter(pred);
                        }

                        in = meet(in, predOut, node, currentContext);
                    }
                    ...
                    if (!programRepresentation().resolveTargets(currentContext.getMethod(), node).isEmpty()) {
                        for (M targetMethod : programRepresentation().resolveTargets(currentContext.getMethod(), node)) {
                            ...
                            if (targetContext.isAnalysed()) {
                                ...
                                out = meet(out, returnedValue, node, currentContext);
                            }
                        }
                        ...
                        if (hit) {
                            ...
                            out = meet(out, localValue, node, currentContext);
                        } else {
                ...
                out = meet(out, prevOut, node, currentContext);

                // Set the OUT value
                currentContext.setValueAfter(node, out);

                boolean branchOUtConverged = true;
                for (N successor : currentContext.getControlFlowGraph().getSuccsOf(node)) {
                    Boolean hsConverged = currentContext.getEdgeConvergenceValue(node, successor);
                    if (hsConverged != null)
                        branchOUtConverged = hsConverged && branchOUtConverged;
                }

                // If OUT has changed...
                if (out.equals(prevOut) == false || !branchOUtConverged) {
                ...
                for (N tailNode : currentContext.getControlFlowGraph().getTails()) {
                    ...
                    exitValue = meet(exitValue, tailOut, tailNode, currentContext);
                }
