			...
			if (node != null) {
				...
				if (successors.size() != 0) {
					...
					for (N succ : successors) {
						...
						out = meet(out, succIn,node,currentContext);
					}					
				...
				if (programRepresentation().isCall(node)) {
					...
					for (M targetMethod : programRepresentation().resolveTargets(currentContext.getMethod(), node)) {
						...
						if (targetContext.isAnalysed()) {
							...
							in = meet(in, callValue,node,currentContext);
					...
					if (hit) {
						...
						in = meet(in, localValue,node,currentContext);
					}
				} else {
				...
				in = meet(in, prevIn,node,currentContext);
				...
				for (N headNode : currentContext.getControlFlowGraph().getHeads()) {
					...
					entryValue = meet(entryValue, headIn,headNode,currentContext);
				}
