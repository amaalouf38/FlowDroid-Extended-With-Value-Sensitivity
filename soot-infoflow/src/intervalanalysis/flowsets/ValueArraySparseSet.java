 ...
 */public class ValueArraySparseSet extends AbstractFlowSet {
	...
	public void removeIntervalFor(soot.Local local) {
		int i = 0;
		while (i < this.numElements) {
			if (((InterVal)elements[i]).l.getName()==local.getName()) {
				elements[i] = elements[--numElements];
				return;
			} else
				i++;
	...
	public void merge(ValueArraySparseSet in,boolean isDummy)
	{
		...
		while (i < in.numElements) {
			fInterVal curFInterval=(fInterVal)in.elements[i];
			soot.Local l=new soot.jimple.internal.JimpleLocal(curFInterval.l.getName(), soot.IntType.v());
			InterVal inter = new InterVal(l, curFInterval.lower, curFInterval.upper, curFInterval.bottom, curFInterval.top,isDummy);
			this.add(inter);
	...
	public boolean hasBottom() {
		int i = 0;
		while (i < this.numElements) {
			if(((InterVal)elements[i]).isBottom()) {
				return true;
			}
			i++;
		}
	...
	public void setBottom() {
		int i = 0;
		while (i < this.numElements) {
			((InterVal)elements[i]).bottom=true;
			i++;
		}
